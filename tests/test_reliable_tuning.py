...
class ReliableTuningTests(unittest.TestCase):
    def setUp(self):
        ...
        self.configs = [
            ScheduleConfig(32, 32, 32, 4, 2),
            ScheduleConfig(64, 64, 32, 4, 3),
        ]

    def test_graph_timer_reports_per_call_units_and_replays_without_python_launches(
        self,
    ):
        torch = MagicMock()
    ...
    def test_timer_rejects_bad_settings_before_touching_runtime(self):
        for kwargs in (
            ...
            with self.subTest(kwargs=kwargs), self.assertRaises(ValueError):
                triton_backend.time_cuda_callable(
                    MagicMock(), MagicMock(), "cuda", **kwargs
                )
    ...
    def test_search_filters_failed_and_nonfinite_results_and_keeps_full_history(self):
        ...
        with self.assertRaisesRegex(RuntimeError, "valid"):
            random_search(
                SequenceBenchmark([Measurement(math.nan), Measurement(math.inf)]),
                self.workload,
                self.configs,
                2,
            )
    ...
    def test_environment_has_context_history_masks_and_scale_invariant_reward(self):
        for scale in (1.0, 100.0):
            env = TuningEnvironment(
                SequenceBenchmark([Measurement(8 * scale)]),
                self.workload,
                self.configs,
                2,
                baseline_latency_us=10 * scale,
            )
    ...
    def test_invalid_measurement_spends_budget_without_improving_incumbent(self):
        env = TuningEnvironment(
            SequenceBenchmark([Measurement(0.1, valid=False), Measurement(math.nan)]),
            self.workload,
            self.configs,
            2,
    ...
    def test_environment_rejects_empty_or_invalid_budget_and_baseline(self):
        for budget, candidates, baseline in (
            (0, self.configs, 10),
            (1, [], 10),
            (1, self.configs, math.nan),
        ):
            with self.assertRaises(ValueError):
                TuningEnvironment(
                    SequenceBenchmark([]),
