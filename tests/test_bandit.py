...
class BanditTests(unittest.TestCase):
    def test_online_update_matches_batch_ridge_solution(self):
        ...
        np.testing.assert_allclose(
            model.inverse, np.linalg.inv(expected_a), rtol=1e-10, atol=1e-10
        )
        np.testing.assert_allclose(
            model.weights, np.linalg.solve(expected_a, x.T @ y), rtol=1e-10
        )

    def test_context_changes_the_preferred_action(self):
        model = LinearUCB(2, alpha=0)
        for _ in range(20):
            model.update(np.array([1.0, 0.0]), 1)
            model.update(np.array([0.0, 1.0]), -1)
    ...
    def test_exploration_and_masking(self):
        ...
        model.update(np.array([1.0, 0.0]), 0)
    ...
    def test_snapshot_roundtrip_and_episode_isolation(self):
        model = LinearUCB(2, alpha=0.2)
        model.update(np.array([1.0, 2.0]), 0.4)
        with TemporaryDirectory() as directory:
            ...
            copied = loaded.copy()
            copied.update(np.array([1.0, 0.0]), 3)
            np.testing.assert_allclose(loaded.weights, model.weights)
    ...
    def test_features_cover_workload_config_and_tail_dimensions(self):
        ...
        aligned = context_features(
            Workload(KernelKind.MATMUL, (1024, 1024, 1024)), config
        )
        irregular = context_features(
            Workload(KernelKind.MATMUL, (1000, 769, 513)), config
        )
        changed = context_features(
            Workload(KernelKind.MATMUL, (1024, 1024, 1024)),
        ...
        with self.assertRaises(ValueError):
            context_features(
                Workload(KernelKind.MATMUL, (1, 1, 1)), ScheduleConfig(17, 32, 32, 4, 2)
            )
