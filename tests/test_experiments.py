...
class ExperimentTests(unittest.TestCase):
    def test_strict_json_keeps_failure_detail_and_replaces_nonfinite_numbers(self):
        with TemporaryDirectory() as directory:
            ...
            experiments.write_json(path, {'failed': Measurement(math.inf, valid=False, detail='resource limit')})
    ...
    def test_summary_uses_independent_confirmation_and_paired_speedups(self):
        case = {'confirmation': [
            {'measurements': {'pytorch': {'latency_us': 10, 'valid': True}, 'random': {'latency_us': 5, 'valid': True}}},
            {'measurements': {'pytorch': {'latency_us': 20, 'valid': True}, 'random': {'latency_us': 10, 'valid': True}}},
        ]}
