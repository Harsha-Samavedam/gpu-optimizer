...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare GPU autotuning search policies."
    )
    subparsers = parser.add_subparsers(dest="command", required=True)
    compare = subparsers.add_parser("compare")
    compare.add_argument(
        "--kernel", choices=[k.value for k in KernelKind], default="matmul"
    )
    compare.add_argument("--shape", default="1024,1024,1024")
    compare.add_argument("--dtype", default="fp16")
    compare.add_argument(
        "--backend", choices=("simulated", "triton"), default="simulated"
    )
    ...
    compare.add_argument(
        "--timing-method", choices=("cuda_graph", "events"), default="cuda_graph"
    )
    compare.add_argument("--graph-batch-size", type=int, default=512)
    compare.add_argument("--device", default=None)
    compare.add_argument(
        "--oracle",
        action="store_true",
        help="also exhaustively evaluate every candidate",
    )
    compare.add_argument(
        "--results-json",
        type=Path,
        default=None,
        help="write complete comparison results to this file",
    )
    preflight = subparsers.add_parser(
        "preflight", help="check CUDA/Triton availability on this machine"
    )
...
def main() -> None:
    ...
    random_result = random_search(
        benchmark, workload, candidates, args.budget, args.seed
    )
    print(
        f"workload: {kernel.value} {shape} {args.dtype}; candidates: {len(candidates)}"
    )
    ...
    print(
        f"random ({random_result.evaluations} evals): {random_result.measurement.latency_us:.3f} us | {random_result.config}"
    )
    oracle_result = None
    if args.oracle:
        oracle_result = exhaustive_search(benchmark, workload, candidates)
        print(
            f"oracle ({oracle_result.evaluations} evals): {oracle_result.measurement.latency_us:.3f} us | {oracle_result.config}"
        )
    if args.results_json is not None:
        results = {
            ...
            "device_info": asdict(benchmark.device_info())
            if args.backend == "triton"
            else None,
            "pytorch_cublas": asdict(torch_baseline)
            if torch_baseline is not None
            else None,
            "fixed": {
                "config": asdict(fixed_config),
                "measurement": asdict(fixed_measurement),
            },
            "random": asdict(random_result),
            "oracle": (asdict(oracle_result) if oracle_result is not None else None),
        }
        from .experiments import write_json

        write_json(args.results_json, results)
